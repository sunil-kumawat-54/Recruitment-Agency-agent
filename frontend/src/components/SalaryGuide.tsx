...
import { DollarSign, MessageSquare, Mail, Copy, Check, TrendingUp, AlertOctagon, HelpCircle } from 'lucide-react';
...
export const SalaryGuide: React.FC = () => {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      toast.success('Negotiation benchmarks ready!');
    } catch (err: any) {
      console.error(err);
  ...
  const handleCopyEmail = () => {
    ...
    setIsCopied(true);
    toast.success('Counter-offer email copied to clipboard!');
    ...
    <div className="space-y-8 max-w-5xl mx-auto">
      ...
      <div>
        ...
        <p className="text-brand-gray text-sm leading-relaxed">
          Verify your compensation benchmarks using localized city multipliers. Access anchoring techniques, verbal playbooks, and copy-paste counter emails.
        </p>
      </div>
      ...
      <div className="glass-panel p-6 rounded-2xl border border-brand-border space-y-6">
        <h3 className="text-sm font-bold uppercase tracking-wider text-slate-400 border-b border-brand-border/60 pb-3">
          Provide Salary Assessment Data
        </h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-2">
                Received Job Offer Compensation (Optional)
              </label>
              <input
                type="text"
                value={receivedOffer}
        ...
        <div className="space-y-8 animate-fadeIn">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="glass-panel p-5 rounded-2xl border border-brand-border text-center">
              ...
              <span className={`text-2xl font-black ${getVerdictColor(salaryReport.negotiation_verdict)}`}>
          ...
          {/* Phrases playbook & Counter offer email templates */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-xs leading-relaxed text-slate-300">
                ...
                <div>
                  ...
                  <ul className="space-y-2">
                    {salaryReport.phrases_to_use?.map((phrase: string, i: number) => (
                      <li key={i} className="bg-brand-emerald/[0.02] border border-brand-emerald/10 p-2.5 rounded-lg italic text-slate-200">
                        "{phrase}"
                      </li>
                    ))}
                  </ul>
                ...
                <div>
                  ...
                  <ul className="space-y-2">
                    {salaryReport.phrases_to_avoid?.map((phrase: string, i: number) => (
                      <li key={i} className="bg-brand-rose/[0.02] border border-brand-rose/10 p-2.5 rounded-lg italic text-slate-400 line-through">
                        "{phrase}"
                      </li>
                    ))}
                  </ul>
                ...
                <div className="pt-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {salaryReport.non_salary_benefits_to_negotiate?.map((benefit: string) => (
                      <span key={benefit} className="bg-slate-900 border border-brand-border px-2.5 py-1 rounded-lg text-[10px] font-semibold">
                        {benefit}
                      </span>
                    ))}
                  </div>
            ...
            <div className="glass-panel p-6 rounded-2xl border border-brand-border space-y-4 flex flex-col justify-between">
              ...
              <textarea
                ...
                className="w-full bg-slate-950/70 border border-brand-border/80 rounded-xl p-4 text-[11px] font-mono leading-relaxed text-slate-300 h-96 focus:outline-none resize-none"
              />
            </div>
          </div>
        </div>
